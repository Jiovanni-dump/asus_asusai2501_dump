...
if [ "${rawdump_enable}" = "1" ]; then
	...
	then
		...
		dd if=/dev/block/bootdevice/by-name/rawdump of=/data/temp/RawRamDump.bin
		echo "ASDF: save rawdump done." > /dev/kmsg
		if [ -f  $SAVE_DUMP_PATH/RawRamDump.tgz ]; then
			rm $SAVE_DUMP_PATH/RawRamDump.tgz
		fi
		tar -czvf $SAVE_DUMP_PATH/RawRamDump.tgz /data/temp/RawRamDump.bin
		echo "ASDF: tar dump DONE." > /dev/kmsg

		rm /data/temp/RawRamDump.bin
